...
import { db } from '../db';
import { profiles, userStatuses } from '../schema';
...
function shapeStatus(row: typeof userStatuses.$inferSelect, profile?: typeof profiles.$inferSelect) {
...
router.get('/', requireAuth, async (req, res) => {
  try {
    const now = Date.now();
    const rows = await db.select().from(userStatuses).orderBy(desc(userStatuses.id));
    const userIds = Array.from(new Set(rows.map((row) => rawObject(row.rawData).userId || rawObject(row.rawData).user_id).filter(Boolean)));
    const profileRows = userIds.length ? await db.select().from(profiles) : [];
...
router.post('/', requireAuth, async (req, res) => {
  try {
    ...
    if (typeof mediaUrl !== 'string' || !mediaUrl.trim()) {
      res.status(400).json({ success: false, error: 'Media URL is required' });
      return;
    }
    if (!['image', 'video'].includes(type)) {
      res.status(400).json({ success: false, error: 'Status type must be image or video' });
      return;
    }
    ...
    await db.insert(userStatuses).values({ id: status.id, rawData: status });
...
router.post('/:id/view', requireAuth, async (req, res) => {
  try {
    const [row] = await db.select().from(userStatuses).where(eq(userStatuses.id, req.params.id)).limit(1);
    if (!row) {
    ...
    await db.update(userStatuses).set({ rawData: { ...raw, viewers } }).where(eq(userStatuses.id, row.id));
...
router.post('/:id/like', requireAuth, async (req, res) => {
  try {
    ...
    await db.update(userStatuses).set({ rawData: { ...raw, likes: nextLikes } }).where(eq(userStatuses.id, row.id));
...
router.delete('/:id', requireAuth, async (req, res) => {
  try {
    ...
    const raw = rawObject(row.rawData);
    if (raw.userId !== res.locals.auth.userId && raw.user_id !== res.locals.auth.userId) {
      res.status(403).json({ success: false, error: 'Forbidden' });
      return;
    }
    await db.delete(userStatuses).where(eq(userStatuses.id, row.id));
